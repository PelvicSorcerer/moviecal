...
A Claude worker is invoked in structured-output `dontAsk` mode with an explicit tool set (`--tools`/`--allowedTools`, MOV-386; `worker-startup-check.mjs` confirms both from the worker's own `system/init` event) and Codex in structured-output `workspace-write --ask-for-approval never` mode. Both are wrapped in the same inherited macOS sandbox; vendor-specific controls are defense in depth. Getting a headless Claude session to run without hanging still requires pre-trusting the repo's main checkout path in `~/.claude.json` (`claude-trust.mjs`, wired into `WorktreeManager.create()`).
