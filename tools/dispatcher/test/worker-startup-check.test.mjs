...
const init = (overrides = {}) => ({
  type: "system", subtype: "init", permissionMode: "default", tools: [...CLAUDE_WORKER_TOOLS], claude_code_version: "2.1.281", ...overrides,
});

describe("evaluateClaudeInit (MOV-386)", () => {
  it("matches default with exactly the allowlisted tools", () => {
  ...
  it("flags an unexpected permission mode", () => {
    const check = evaluateClaudeInit(init({ permissionMode: "acceptEdits" }));
    expect(check).toMatchObject({ status: "mismatch", permissionMode: "acceptEdits", unexpectedTools: [] });
    expect(check.problems).toEqual(['permissionMode is "acceptEdits", expected "default"']);
  });
  ...
  it("treats an absent mode or tools list as a mismatch", () => {
    expect(evaluateClaudeInit(init({ permissionMode: undefined })).problems).toEqual(['permissionMode is absent, expected "default"']);
...
describe("reportClaudeStartupCheck (MOV-386)", () => {
  it("logs a loud warning naming the problem on mismatch", () => {
    ...
    expect(reportClaudeStartupCheck(evaluateClaudeInit(init({ permissionMode: "acceptEdits" })), { label: "MOV-1", logger })).toBe(true);
    expect(logger.warn).toHaveBeenCalledWith(expect.stringMatching(/^WARNING \(MOV-386\): Claude worker for MOV-1 did not start as configured — permissionMode is "acceptEdits"/));
  ...
  it("logs one quiet confirmation on a match", () => {
    ...
    expect(logger.log).toHaveBeenCalledWith("Claude worker for MOV-3 started in default mode with the allowlisted tools only.");
  });
});
...
describe("makeInitEventWatcher (MOV-386)", () => {
  it("fires once for the first init event, across chunk boundaries", () => {
    ...
    const text = [JSON.stringify({ type: "assistant" }), "not json", JSON.stringify(init()), JSON.stringify(init({ permissionMode: "acceptEdits" }))].join("\n") + "\n";
    watch(Buffer.from(text.slice(0, 40)));
    watch(Buffer.from(text.slice(40)));
    expect(onInit).toHaveBeenCalledTimes(1);
    expect(onInit.mock.calls[0][0].permissionMode).toBe("default");
  });
});
...
describe("describeClaudeStartupCheck — doctor rendering (MOV-386)", () => {
  ...
  it("passes with the last observed mode, tools and CLI version", () => {
    ...
    expect(result.detail).toBe(
      "matches — last observed MOV-10 at 2026-09-26T10:00:00.000Z, Claude Code 2.1.281: mode default; tools Read, Edit, Write, Glob, Grep, Bash, NotebookEdit, Task.",
    );
  });

  it("fails on a recorded mismatch and names what differed", () => {
    const result = describeClaudeStartupCheck([run("MOV-10", init()), run("MOV-11", init({ permissionMode: "acceptEdits", tools: ["Read", "Workflow"] }))]);
    expect(result.ok).toBe(false);
    expect(result.detail).toMatch(/^MISMATCH — permissionMode is "acceptEdits", expected "default"; tools outside the allowlist: Workflow\. Last observed MOV-11/);
    expect(result.detail).toContain("mode acceptEdits; tools Read, Workflow");
  });
