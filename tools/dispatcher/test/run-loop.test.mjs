...
describe("iOS Companion App worker-lane simulator lease (MOV-311)", () => {
  ...
  it("never touches the lease for a non-iOS issue", async () => {
    const acquireIosSimLeaseFn = vi.fn();
    const releaseIosSimLeaseFn = vi.fn();
    const ctx = baseCtx({ acquireIosSimLeaseFn, releaseIosSimLeaseFn });

    const [result] = await runOnce([ISSUE], ctx);

    expect(result.outcome).toBe("in-review");
    expect(acquireIosSimLeaseFn).not.toHaveBeenCalled();
    expect(releaseIosSimLeaseFn).not.toHaveBeenCalled();
  });

  it("acquires the lease before dispatching an iOS Companion App issue, passes its id to the worker, and releases it on success", async () => {
    const acquireIosSimLeaseFn = vi.fn(async () => ({ acquired: true, lease: { id: "lease-abc" } }));
    const releaseIosSimLeaseFn = vi.fn(async () => {});
    const ctx = baseCtx({ acquireIosSimLeaseFn, releaseIosSimLeaseFn });

    const [result] = await runOnce([IOS_ISSUE], ctx);

    expect(result.outcome).toBe("in-review");
    expect(acquireIosSimLeaseFn).toHaveBeenCalledWith(IOS_ISSUE);
    expect(ctx.spawnWorkerFn.mock.calls[0][0].iosSimLeaseId).toBe("lease-abc");
    expect(releaseIosSimLeaseFn).toHaveBeenCalledWith("lease-abc");
  });

  it("defers silently (no Linear write, no worktree) when the lease is held by another lane", async () => {
    const acquireIosSimLeaseFn = vi.fn(async () => ({ acquired: false, reason: "the manual lane holds lease lease-x" }));
    const releaseIosSimLeaseFn = vi.fn();
    const ctx = baseCtx({ acquireIosSimLeaseFn, releaseIosSimLeaseFn });

    const [result] = await runOnce([IOS_ISSUE], ctx);

    expect(result).toEqual({ issue: "MOV-1", outcome: "deferred-ios-sim-lease", reason: "the manual lane holds lease lease-x" });
    expect(ctx.worktreeManager.createCalls).toHaveLength(0);
    expect(ctx.spawnWorkerFn).not.toHaveBeenCalled();
    expect(ctx.linearClient.calls).toEqual([]);
    expect(releaseIosSimLeaseFn).not.toHaveBeenCalled();
  ...
  it("releases the lease on a worker failure just as it does on success", async () => {
    const acquireIosSimLeaseFn = vi.fn(async () => ({ acquired: true, lease: { id: "lease-fail" } }));
    const releaseIosSimLeaseFn = vi.fn(async () => {});
    const ctx = baseCtx({
      acquireIosSimLeaseFn,
      releaseIosSimLeaseFn,
      spawnWorkerFn: vi.fn(async () => ({ exitCode: 1, logDir: "/fake/logs/x" })),
      auditWorkerResultFn: vi.fn(() => ({ ok: true, violations: [] })),
    });
    ...
    expect(result.outcome).not.toBe("in-review");
    expect(releaseIosSimLeaseFn).toHaveBeenCalledWith("lease-fail");
  });

  it("still releases the lease when the preflight iOS-runner gate fails before a worktree is ever created", async () => {
    const acquireIosSimLeaseFn = vi.fn(async () => ({ acquired: true, lease: { id: "lease-preflight" } }));
    const releaseIosSimLeaseFn = vi.fn(async () => {});
    const ctx = baseCtx({ acquireIosSimLeaseFn, releaseIosSimLeaseFn, iosRunnerOnline: false });

    const [result] = await runOnce([IOS_ISSUE], ctx);

    expect(result.outcome).toBe("blocked");
    expect(ctx.worktreeManager.createCalls).toHaveLength(0);
    expect(releaseIosSimLeaseFn).toHaveBeenCalledWith("lease-preflight");
  });

  it("releases the lease on a worker timeout", async () => {
    vi.useFakeTimers();
    try {
      const acquireIosSimLeaseFn = vi.fn(async () => ({ acquired: true, lease: { id: "lease-timeout" } }));
      const releaseIosSimLeaseFn = vi.fn(async () => {});
      const spawnWorkerFn = vi.fn(() => new Promise(() => {})); // never resolves
      const ctx = baseCtx({ acquireIosSimLeaseFn, releaseIosSimLeaseFn, spawnWorkerFn, workerTimeoutMs: 1000 });

      const runPromise = runOnce([IOS_ISSUE], ctx);
      await vi.advanceTimersByTimeAsync(1000);
      const [result] = await runPromise;

      expect(result.outcome).toBe("timeout");
      expect(releaseIosSimLeaseFn).toHaveBeenCalledWith("lease-timeout");
    } finally {
      vi.useRealTimers();
    }
  });

  it("releases the lease when the attempt stops at a safe boundary", async () => {
    const acquireIosSimLeaseFn = vi.fn(async () => ({ acquired: true, lease: { id: "lease-stop" } }));
    const releaseIosSimLeaseFn = vi.fn(async () => {});
    let releaseWorker;
    const workerPromise = new Promise((resolve) => {
      releaseWorker = () => resolve({ exitCode: 0, logDir: "/fake/logs/x" });
    ...
    const ctx = baseCtx({
      ...
      refreshIssueFn: vi.fn(async () => {
        refreshCount += 1;
        return refreshCount === 1 ? { ...IOS_ISSUE, stateName: "Ready for Agent" } : { ...IOS_ISSUE, stateName: "Canceled" };
      }),
    });
