...
// Machine-wide iOS simulator lease (MOV-309).
//
// CI (`lane-ios`), dispatcher workers, and human-led manual testing share one
// Mac with 8 GB of RAM. Each lane gets its own simulator device so installed
// binaries, app data, and Keychain state stop clobbering each other, and this
// lease keeps at most one of those devices booted — and serializes heavy
// `xcodebuild` work with it — because RAM, not CPU, is the binding resource.
//
// The policy (lane detection, expiry, FIFO, takeover, unmanaged-use rules) lives
// in ./lib/ios-sim-lease-core.mjs and is pure. This file owns the I/O: the
// durable record, the mutex, `simctl`, the process table, and the CLI. It
// re-exports the policy so callers and tests have one entry point.
