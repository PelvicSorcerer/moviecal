// Machine-wide iOS simulator lease policy (MOV-309).
//
// Everything here is pure: it decides *what* should happen to the lease record,
// never touching the clock, the filesystem, or `simctl`. The CLI
// (scripts/ios-sim-lease.mjs) injects all three, which is what makes the expiry,
// FIFO, takeover, and unmanaged-use rules testable without a real simulator.
