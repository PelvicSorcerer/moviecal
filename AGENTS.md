...
- **Two execution adapters.** Linear owns desired lifecycle state; GitHub owns delivered state. Eligible non-iOS work may execute on a Linear-managed **cloud** adapter; iOS/Xcode work — and anything needing a local secret or the self-hosted runner — runs on the **Mac** adapter (`docs/operators/local-execution.md`). Both satisfy one behavioral contract: same branch conventions, same PR template, same required checks, same hard-deny boundaries. Every executable issue carries one explicit, provisioned `execution:{cloud,mac,none}` label before dispatch (MOV-142/MOV-143). See `docs/governance/hybrid-execution-architecture.md`. **The cloud adapter is decided but not enabled** — today every implementation issue still runs on the Mac adapter.
