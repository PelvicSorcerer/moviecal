...
- Membership exit is one shared contract, not a per-surface rule: `leaveSharedWatchlist` lets an accepted editor remove only their own membership, `removeSharedWatchlistMember` stays owner-only, and both refuse any row that is (or claims to be) the owner's. `removeSharedWatchlistMember` resolves the target membership scoped to the watchlist before deleting anything, so an id crafted from another list, or the non-uuid synthetic `owner:<user-id>` id above, reads as "not found" rather than reaching the delete or surfacing a database error. Cookie and bearer routes are expected to call these operations rather than re-deriving the rules; access loss is recomputed per request, so a removed or departed member loses list detail, mutation, watchlist targeting, and their list-only calendar contributions on their next request, and rejoining needs a still-valid invite.
- Member email addresses are the only private account metadata a shared list holds about other people, and they are owner-only. `listSharedWatchlistMemberProfiles` proves ownership before it resolves a single account, and the lookup itself runs through the service-role admin client, which exists server-side only. `listSharedWatchlistMembers` remains the email-free listing for callers that need roles alone.
