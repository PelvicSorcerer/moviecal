...
**Turns are not comparable across harnesses.** Claude's `num_turns` counts model-request/tool round trips inside one session; Codex's count is the number of `turn.completed` events, and a Codex turn can span many tool calls. `turnsSource` names the unit. Compare duration and accepted issue outcomes across workers, and compare turns only within one worker.
...
Fresh `worker:any` issues resolve to Claude (there is no quota balancing). A bounded, disabled-by-default trial can temporarily send them to Codex while preserving each issue's tier and pinned model/effort; the full policy, states and rollback are in `docs/operators/worker-routing.md` §Temporary `worker:any` → Codex trial. Operationally:
...
- **Admission and the cap.** The run loop, which holds the singleton dispatcher lock, admits each fresh assignment immediately before creating its worktree, re-reading expiry and the cap then. The ledger is keyed by issue identifier, so a restart or a duplicate/retried admission returns the existing record and never consumes a second slot. A Codex cooldown leaves trial-routed issues queued; a trial that ends between batch start and admission defers the issue (`deferred-worker-trial-ended`) to be re-resolved under the baseline on the next poll.
...
- **Worker quota-pool cooldown (MOV-360).** MOV-151/192/205 above are all *per-issue*: each deferral, resume, or escalation is scoped to the one issue whose attempt hit the limit. That left a gap the 2026-09-25 incident exposed — five Claude issues, each correctly deferred individually, still burned through most of the `Ready for Agent` queue one at a time within a couple of minutes, because nothing stopped the *next* Claude issue from claiming the concurrency slot the prior one's refusal had just freed. `tools/dispatcher/src/worker-cooldown.mjs`'s `WorkerCooldownStore` closes that gap with a second, independent, worker-scoped record (one per quota pool — Claude, Codex — persisted at `~/.config/moviecal/worker-cooldowns.json`, surviving a restart the same way `usage-limits.json` does) that gates *dispatch* rather than any one issue's retry.
  ...
  Before worktree creation, the dispatcher gates each issue's resolved worker. A fresh worker:any issue retains Claude (or Codex while the MOV-383 trial is active, through this same gate); a deferred retry or retained resume keeps its recorded provider. The gate never selects another provider. After reset, one eligible same-provider attempt is admitted as a probe, preferring a due retry/resume. A live cooldown recheck after waiting for the single worker slot prevents a preceding refusal from cascading through the batch.
...
- **Claude permission mode, the scrub, and the explicit tool set (MOV-386).** `CLAUDE_CODE_SUBPROCESS_ENV_SCRUB=1` stays set; it is a credential hardening and is never opted out of. Claude Code (observed on 2.1.281) treats that scrub as incompatible with an *implicit* tool set: without an explicit `allowedTools` declaration it silently forces the session from the requested `--permission-mode dontAsk` back to `default` (stderr: `Permission mode forced to default — CLAUDE_CODE_SUBPROCESS_ENV_SCRUB is set`) and loads every built-in tool, including `Workflow`, `Cron*`, `ScheduleWakeup`, `RemoteTrigger`, `SendMessage`, `PushNotification`, `WebFetch`, `WebSearch`, `EnterWorktree`/`ExitWorktree`, `DesignSync`, and `Monitor`. `default` mode is not safe for a headless process: a call needing approval has nobody to answer it. `workerInvocation()` therefore declares `CLAUDE_WORKER_TOOLS` (`tools/dispatcher/src/worker-routing.mjs`: `Read`, `Edit`, `Write`, `Glob`, `Grep`, `Bash`, `NotebookEdit`, `Task`) twice. `--tools` makes it the only available set, so nothing else is loaded. `--allowedTools` makes it the permission allowlist, which keeps `dontAsk` in force. The worker-only deny list in the `--settings` payload still applies on top, and Claude evaluates deny before allow, so `Bash(git*)`, `Bash(curl*)` and the rest are refused even though `Bash` as a whole is allowed. Under `dontAsk`, a refused or unmatched call fails immediately instead of waiting for a prompt. Repair workers use the same invocation. `Task` remains so a worker can delegate a broad search to a built-in subagent, which draws on the same session's tools, mode, and denies. (`--safe-mode` already disables the repository's custom `.claude/agents`.) `TodoWrite` is deliberately not listed because it could not be confirmed present on the installed CLI. Allowing the whole `Bash` tool is broader than the project `.claude/settings.json` Bash allowlist that `dontAsk` alone would enforce. It is bounded by the deny list, the Seatbelt profile, and the post-run transcript and diff audits, which remain the authoritative boundaries.
- **Startup check (MOV-386).** Claude's own stream-json `system/init` event is the only reliable record of the mode and tools a session actually got. `spawnWorker()` hands the first init event to `worker-startup-check.mjs` as soon as it appears. If `permissionMode` is not `dontAsk` or any tool outside `CLAUDE_WORKER_TOOLS` is present (MCP tools included), the dispatcher log gets a `WARNING (MOV-386)` line naming exactly what differed. The run is **not** killed. Each usage record (`usage.json` and the worker-usage ledger) carries a `startupCheck` object: `status` is `match`, `mismatch`, or `missing` (no init event); it also stores the observed mode and tools, any unexpected or unloaded tools, and the Claude Code version. `dispatcher doctor` prints the last observed mode, tool set, and CLI version, and fails that check when the last observed run was a mismatch.
- **Network tools and the Seatbelt profile.** The `worker-guard.mjs` profile is `(allow default)` plus specific denies. It blocks *execution* of `curl`, `ssh`/`scp`/`sftp`, Git, `gh`, and deploy binaries, but it does not deny network sockets. The Claude process itself needs outbound HTTPS to reach the model, and `WebSearch` runs provider-side. So `WebFetch`/`WebSearch` were **not** blocked at the OS level before MOV-386. Removing them from the available tool set is a real control, not only defence in depth. An arbitrary script (for example `node -e "fetch(...)"`) can still open a connection. The post-run audit and the dispatcher-only Git/publication boundary are what keep that from becoming a remote mutation.
