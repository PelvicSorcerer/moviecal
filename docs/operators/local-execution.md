...
**`lane-review` (added 2026-09-08, not yet a required status check):** `scripts/lane-review.mjs`, run by `.github/workflows/review-verify.yml` on every PR. Two layers: heuristic checks (sensitive paths — `.github/workflows/**`, `AGENTS.md`, `docs/product/**`, ruleset-shaped filenames; secret-shaped strings; diff-size threshold) that always run and need no credential; an AI review pass (a fresh Claude call, strict reviewer-only prompt) that runs only when the `ANTHROPIC_API_KEY` repo secret is present — its absence is a warning, not a failure, so this lane is safe to make required before that secret exists. It fails the check (never posts a GitHub review/approval, only a plain PR comment for visibility) on any blocking finding. Verified locally against three cases: a diff touching `.github/workflows/**` (correctly blocks — confirmed against the PR that introduced this very lane), a diff containing a GitHub-PAT-shaped string (correctly blocks), and a benign diff (correctly passes). **Deliberately not yet added to `master-protection`'s required checks** — it's running informationally first so its false-positive rate can be observed on real PRs before it can block merges; adding it as required is a separate, explicit follow-up.
