...
- **Worker quota-pool cooldown (MOV-360).** MOV-151/192/205 above are all *per-issue*: each deferral, resume, or escalation is scoped to the one issue whose attempt hit the limit. That left a gap the 2026-09-25 incident exposed — five Claude issues, each correctly deferred individually, still burned through most of the `Ready for Agent` queue one at a time within a couple of minutes, because nothing stopped the *next* Claude issue from claiming the concurrency slot the prior one's refusal had just freed. `tools/dispatcher/src/worker-cooldown.mjs`'s `WorkerCooldownStore` closes that gap with a second, independent, worker-scoped record (one per quota pool — Claude, Codex — persisted at `~/.config/moviecal/worker-cooldowns.json`, surviving a restart the same way `usage-limits.json` does) that gates *dispatch* rather than any one issue's retry.
  ...
  Gating happens once per poll batch, before worktree creation: `runOnce()` snapshots both workers' cooldown state up front and resolves, once, which worker each issue in the batch would use — a pinned issue always uses its pin; a `worker:any` issue that already began an attempt stays bound to the worker recorded on its own `usage-limits.json` entry (`resolveDispatchWorker()`, `worker-routing.mjs`); a fresh `worker:any` issue picks an available worker (Claude by default, Codex when Claude is cooling, and symmetrically) or is left queued unclaimed if neither is. An issue resolving to a *cooling* worker is skipped silently — no worktree, no worker, no Linear write, exactly like the per-issue deferral's own silence, since the reason was already published once when the cooldown was set. Once a worker's reported reset has passed but no post-reset attempt has told the store the outcome yet (`probeOwed`), exactly one same-batch issue using it is admitted as that probe — preferring one with a due per-issue retry or MOV-205 resume for that worker over a fresh claim, when one is eligible — and every other same-worker issue keeps waiting; a fresh `worker:any` issue re-resolves live at its own turn (not against the stale batch-start snapshot), so it correctly follows a worker an earlier same-batch issue's outcome just freed up instead of giving up on a pick that went stale mid-batch. A same-batch issue that discovers a *new* cooldown is what stops the rest of that batch from burning through the same dead worker one issue at a time, single-flight concurrency permitting.

  This never changes the per-issue mechanics above: the existing bounded one-retry-or-resume rule, the second-consecutive-limit escalation, worktree ownership/reclaim, and the route/delegate/safety gates all apply exactly as documented. The cooldown decides *whether* dispatch of an issue is attempted at all; everything after that point is unchanged. `dispatcher dry-run` prints each worker's live state (`open` / `COOLING until <reset>` / `PROBE OWED`) and, per issue, the worker a real dispatch would actually use — a quota-aware `worker:any` pick or an existing binding — without consuming a probe or writing anything; `dispatcher doctor` reports the same per-worker state as an informational check. See `docs/operators/worker-routing.md` §Worker quota-pool cooldown for the operator-facing summary.
