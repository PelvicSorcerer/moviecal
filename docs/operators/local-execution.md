...
- **Two real worktree-reclaim races, found by MOV-198's real-subprocess integration tests (`tools/dispatcher/test/worktree-reclaim-concurrency.integration.test.mjs`) and pinned there as `it.fails(...)` cases pending their own fixes:**
  - `WorktreeManager.isPathFreeForIssue()`'s reclaim path checks a worktree is clean, then — as a separate, later `git worktree remove --force` call — removes it, with no lock held across the gap. A real concurrent process writing into the worktree in that window has its work silently destroyed, since `--force` overrides Git's own "directory is dirty" refusal. See MOV-201.
