...
**Reading the direction back is just as easy to invert (MOV-128).** An issue's own `relations` field of type `blocks` lists issues *it* blocks (its dependents), not its blockers — the same inversion class as the write-side bug above, just on the read path. The issues that actually block a given issue show up under that issue's `inverseRelations` instead. `LinearClient.issuesInState()` derives `blockedByIds` from `inverseRelations`, and `tools/dispatcher/src/dependency-gate.mjs`'s `buildIsIssueSatisfied()` resolves each blocker's workflow state from the same query (no extra Linear call) to gate dispatch in `bin/dispatcher.mjs`'s real run path.
