...
**Researched 2026-09-08:** Linear does have native two-way GitHub sync — but as two distinct mechanisms, neither of which is what we currently have enabled, and both are Settings-UI-only (no public API mutation exists for either, confirmed by schema introspection):

1. **The GitHub Issues Importer** we used offers a separate "sync changes from Linear back to GitHub" option in its own wizard for the issues it imports — distinct from "sync issues after import" (one-way ingestion of new GitHub issues), which is the only option we exercised.
2. **GitHub Issues Sync** (`Settings → Integrations → GitHub`, a standalone feature separate from the Importer) links one Linear team to one GitHub repo, one-way or two-way, but only for **newly created** issues going forward — it does not retroactively cover the 110 already-imported issues.

Enabling either safely requires checking, live in the workspace's Settings, whether it would edit our existing one-way link in place or add an independent, additive one — the latter risks duplicate Linear issues for new GitHub issues (both mechanisms can independently create one). Not something to guess at from documentation alone. **Tracked as [MOV-115](https://linear.app/moviecal/issue/MOV-115)**, labeled `human-only` — do not let this doc's "kept in sync" language above imply that gap is closed; MOV-115 is the source of truth on its status until resolved.
