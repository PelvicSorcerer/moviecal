...
| Repair-budget exhaustion | Further safe repair is refused after the persisted budget is spent, including after restart. | `tools/dispatcher/test/run-loop-e2e.test.mjs`, `tools/dispatcher/test/repair-ledger.test.mjs` | MOV-259 (post-MOV-258): round 1 of a deliberate multi-round budget drill hit a **fourth** distinct guard false positive — `REPAIR_ONLY_RULES` (repair mode's test/governance/dispatcher-code protection) has no read-only carve-out at all, unlike `PATH_RULES`, so a repair worker's own read-only orientation (`npx vitest ...`, `grep ... docs/operators/...`) was treated as a change and the correct fix it produced was refused. The attempt still durably recorded in the repair ledger (`outcome: "failed"`, escalated), consuming 1 of the 2-attempt code-repair budget. [MOV-260](https://linear.app/moviecal/issue/MOV-260) is the held follow-up ([PR #535](https://github.com/PelvicSorcerer/moviecal/pull/535)) | Blocked — MOV-260 |
