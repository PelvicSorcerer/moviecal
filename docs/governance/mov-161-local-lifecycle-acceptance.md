...
| Trusted review repair | A trusted actionable review can trigger the bounded repair lifecycle; untrusted review cannot. | `tools/dispatcher/test/repair-policy.test.mjs` (35 tests pass, including `admitRepair — review triggers`: "repairs on a trusted human REQUEST_CHANGES" / "escalates a REQUEST_CHANGES from an untrusted reviewer" / "repairs on a machine-readable blocking review finding") | MOV-259: live end-to-end is blocked by a genuine operator-environment constraint, not a code defect — GitHub refuses `gh pr review --request-changes` on your own PR ("Can not request changes on your own pull request"), and only one GitHub identity (`PelvicSorcerer`) is available in this environment, so no second login can submit the REQUEST_CHANGES review the scenario needs. The underlying repair-publication pipeline this row would exercise was already proven live end-to-end by the bounded-CI-repair observation (MOV-257); the review-vs-CI trigger classification itself (`classifyReviewTrigger`) is deterministically covered | Observed (deterministic only) — live blocked on single-GitHub-account constraint |
...
| Repair-budget exhaustion | Further safe repair is refused after the persisted budget is spent, including after restart. | `tools/dispatcher/test/run-loop-e2e.test.mjs`, `tools/dispatcher/test/repair-ledger.test.mjs` | MOV-259: after [MOV-260](https://linear.app/moviecal/issue/MOV-260) merged ([PR #535](https://github.com/PelvicSorcerer/moviecal/pull/535)) and the daemon was updated, the same failed audit-blocked attempt (which still durably consumed 1/2 of the code-repair budget, `outcome: "failed"`) was retried on the now-clean worktree and published ("Repair attempt: 2 of 2"), exhausting the budget. A third deliberate failure was then correctly refused: **"Automatic repair stopped and handed PR #533 to a human. Reason: attempt budget exhausted"**, with the stopping reason durably keyed in `~/.config/moviecal/repair-ledger.json` (survives restart, since it is read from disk on every poll — the same durability already exercised by the worker/daemon crash-recovery row) | Observed — MOV-259/260 |
...
## Status: all thirteen scenarios observed

As of 2026-09-18, every row in the transition ledger is `Observed`. Six worker-guard/dispatcher false positives were found and fixed live during the drill, each as its own held-then-merged Linear issue: [MOV-240](https://linear.app/moviecal/issue/MOV-240) (`cat`/`head`-style AGENTS.md reads, PR #502), [MOV-243](https://linear.app/moviecal/issue/MOV-243) (`find -newer` reference argument, PR #506), [MOV-245](https://linear.app/moviecal/issue/MOV-245) (escaped-operator placeholder collision in `canonicalize()`, PR #509), [MOV-258](https://linear.app/moviecal/issue/MOV-258) (required-check detection 404s against this repo's ruleset-protected `master`, PR #531), and [MOV-260](https://linear.app/moviecal/issue/MOV-260) (repair-mode protected-path guard had no read-only carve-out, PR #535). One follow-up remains open and explicitly held: [MOV-254](https://linear.app/moviecal/issue/MOV-254) (an orphaned worker process can outlive a killed daemon and taint a worktree reconciliation just reclaimed) — a real process-supervision gap, not a quick regex fix, deliberately left for a dedicated implementation cycle per the completion rule below. The "trusted review repair" row is observed by deterministic test only; live end-to-end coverage is blocked by a single-GitHub-account operator constraint, not a code defect, and does not need its own follow-up issue since the repair-publication pipeline it would exercise was already proven live by the bounded-CI-repair observation.

Both Linear Loops and `MOVIECAL_AUTO_REPAIR` were returned to their normal (restored / disabled) state after the drills that needed them. The dispatcher daemon runs merged master as of `f634b5a` (MOV-260).
