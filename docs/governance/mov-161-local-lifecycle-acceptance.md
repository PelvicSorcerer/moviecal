...
| Bounded CI repair | A safe CI failure receives no more than the configured repair budget across fresh head SHAs. | `tools/dispatcher/test/run-loop-e2e.test.mjs`, `tools/dispatcher/test/repair-ledger.test.mjs` | MOV-257: pushed a deliberately broken, non-sensitive source+test pair onto a disposable dispatcher-owned PR (#529). Both `lane-unit` and `lane-review` genuinely failed, but `dispatcher repair --dry-run` reported "no actionable failure observed" — the required-check detection 404s against this repo's ruleset-protected `master` (MOV-258, fixed in PR #531). Redrive pending MOV-258 merge + daemon update | Blocked — MOV-258 |
